# Publishing `@hidayahhtaufik/x402-arc` to npm
...
```bash
npm view @hidayahhtaufik/x402-arc
npm view @hidayahhtaufik/x402-arc dist-tags
```
...
node -e "import('@hidayahhtaufik/x402-arc').then(m => console.log(Object.keys(m)))"
```
...
gh release create v0.1.0 \
  --title "v0.1.0 — @hidayahhtaufik/x402-arc on npm" \
  --notes-from-tag
```
...
```
## @hidayahhtaufik/x402-arc v0.1.0
...
run `npx @hidayahhtaufik/x402-arc-facilitator` directly:
...
| Consumer gets "Cannot find module @hidayahhtaufik/x402-arc" | Package didn't publish, or `exports` map is wrong | `npm view @hidayahhtaufik/x402-arc` to confirm; inspect `package.json` exports |
| Need to unpublish a broken version | Can only unpublish within 72h | `npm unpublish @hidayahhtaufik/x402-arc@0.1.0` — then bump and publish a fix |
