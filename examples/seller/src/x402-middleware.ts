 ...
 *
 * Flow:
 *   1. Buyer calls a protected route without `X-PAYMENT` header.
...
// ---- Arc Testnet constants (canonical, verified Day 2) ----
...
function buildPaymentRequirements(
  ...
  return {
    ...
    description: opts.description ?? "Aureus-protected resource",
    payTo: opts.payTo,
...
function decodeXPaymentHeader(headerValue: string): unknown {
  // Header is base64-encoded JSON of the payment payload (per x402 convention).
  try {
...
export function createX402Middleware(opts: X402MiddlewareOptions) {
  ...
  return async function x402(
    ...
    // Verify with the facilitator first (fast pre-check).
    const verifyBody = {
      x402Version: 1,
      paymentPayload,
      paymentRequirements,
    ...
    // Attach settlement info to response so buyer can verify on-chain.
    ...
    // Hand control to the actual route handler.
    next();
  };
}
